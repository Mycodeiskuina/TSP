...
int solveTSP(const vector<vector<int>>& adj,  vector<int>& final_path) {
    ...
    int final_res = INT_MAX;
    TSPRec(adj, curr_bound, 0, 1, curr_path, visited, final_res, final_path);
    */

   // Paralelizar la exploración de cada ciudad desde la ciudad 0
    ...
    for (int i = 1; i < N; ++i) {
        ...
        // Sección crítica para actualizar el mejor resultado global
        #pragma omp critical
        {
    ...
    best_path.push_back(best_path[0]); // cerrar ciclo
...
int main(int argc, char* argv[]) {
    ...
    vector<int> final_path; // to save the optimal route
    ...
    // sec times
    times_out << dimension << " " << end-start << " " <<  th <<"\n"; 
    times_out.close();
